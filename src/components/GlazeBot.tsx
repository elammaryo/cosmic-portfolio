...
export function GlazeBot({ setIsChatOpen }: any) {
  ...
  function ChatWindow() {
    ...
    return (
      <div className="flex flex-col w-full">
        ...
        <div
          className="flex flex-col h-100 gap-5 overflow-auto scroll-smooth [scrollbar-width:none]"
          ref={chatWindow}
        >
  ...
  function ChatMessage({ message, role }: any) {
    function ChatBubble() {
      return (
        <div
          className={cn(
            "whitespace-pre-line text-left max-w-60 p-4 rounded-3xl",
            role === "user"
              ? "bg-primary/80 text-white"
              : "glass glass-tint !shadow-md"
          )}
        >
          {message}
        </div>
      ...
      <div>
        {role === "user" ? (
          <div className="flex gap-3">
            <ChatBubble />
            <div className="flex translate-y-2 rounded-full items-center justify-center glass glass-tint h-12 w-12">
              <UserIcon />
            </div>
          </div>
        ) : (
          <div className="flex gap-3 ">
            <div className="flex translate-y-3 rounded-full items-center justify-center glass glass-tint h-12 w-12">
              <BotMessageSquare />
            </div>
            <ChatBubble />
          </div>
  ...
  return (
    <div className="glass glass-tint max-w-100 max-h-150 pb-5 pt-3 px-5 flex flex-col items-center gap-5 z-50">
      <ChatWindow />
      <ChatInput />
    </div>
  );
